...
echo.
echo === WorkBuddy streak rewards (activity map) ===
echo node : %NODE_BIN%
echo.
rem 先确保常驻 Edge 在跑，再执行领取脚本
"%NODE_BIN%" wb_start_edge.js
echo.
echo Dry run first (nothing is clicked). Type "y" to actually claim, anything else to stop.
set /p ANS=Claim now? [y/N]:
if /i "%ANS%"=="y" (
  echo.
  echo --- APPLY mode: real clicks ---
  "%NODE_BIN%" wb_claim_rewards.js --apply %*
) else (
  echo.
  echo --- DRY RUN ---
  "%NODE_BIN%" wb_claim_rewards.js %*
)
echo.
